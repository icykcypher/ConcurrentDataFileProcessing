...
namespace ConcurrentFileProcessor
{
    public class Program
    {
        private static void Main(string[] args)
        {
            ...
            var processor = new FileProcessor(db, aggregator, config);

            Console.WriteLine("Using DB path: " + config.Database.Path);
            Console.WriteLine("Exists: " + File.Exists(config.Database.Path));

            var watcher = new FileWatcherService(inputDir, queue, config.ProcessedDirectory, config.ErrorDirectory);
            watcher.Start();

            StartWorkers(queue, processor, watcher, config.Workers);
            ...
            var allMeasurements = db.QueryAll();
            aggregator.AddMeasurements(allMeasurements);
